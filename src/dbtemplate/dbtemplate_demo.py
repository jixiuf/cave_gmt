...
def test_single():
    pool1=pools.Pool(
        ...
        max_recycle_sec=3)
    dt=dbtemplate.DatabaseTemplateSingle(pool1)
...
def test_sharding():
    ...
    dt=dbtemplate.DatabaseTemplateSharding([dt0,dt1])
    ...
    yield dt.execSql(dbtemplate.Uint64Sum(1),"insert into test_python_conn_shard value(1)")
    yield dt.execSql(dbtemplate.Uint64Sum(2),"insert into test_python_conn_shard value(2)")
    def mapRow(row):
        return row[0]
    result=yield dt.query(dbtemplate.Uint64Sum(1),"select id from test_python_conn_shard where id=1",mapRow)
    print "shard_result,len(result) should=1",len(result)
    print "shard_result,should=1,actual=",result[0]
    result=yield dt.query(dbtemplate.Uint64Sum(2),"select id from test_python_conn_shard where id=2",mapRow)
    print "shard_result,len(result) should=1",len(result)
