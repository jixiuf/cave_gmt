...
'''
    ...
    @gen.coroutine
    def add(self,mailId,uin,startTime,endTime,awardStr,awardDesc,conent):
        query="insert into MailDraft (mailId,uin,fromUin,mailType,startTime,endTime,awardStr,awardDesc,content,reason) values(%d,%s,0,0,'%s','%s','%s','%s','%s','system')"%(mailId,uin,startTime,endTime,awardStr,awardDesc,conent)
        print(query)
    ...
    @gen.coroutine
    def select_all(self):
        query="select mailId,uin,fromUin,mailType,startTime,endTime,awardStr,awardDesc,content,reason from MailDraft"
        res=yield self.dbtemplate.query(query,self.mapRow)
        raise gen.Return(res)

    @gen.coroutine
    def select_by_mailid(self,mailId):
        query="select mailId,uin,fromUin,mailType,startTime,endTime,awardStr,awardDesc,content,reason from MailDraft where mailid=%d"%(int(mailId))
        res=yield self.dbtemplate.query(query,self.mapRow,mailId)
        raise gen.Return(res)
